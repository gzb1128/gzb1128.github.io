...
- `pubDate` 必须使用带时间的 ISO 格式：`YYYY-MM-DDTHH:MM:SSZ`（如 `2026-06-10T08:00:00Z`）
- 不要使用纯日期格式（`2026-06-10`）。时间精度用于解决同一天发布多篇文章时的排序歧义
- 时间部分用 `T08:00:00Z` 作为默认值（避免 UTC 时区偏移导致日期漂移）
- 同一天内需要控制顺序时，通过调整时间区分先后（如 `T08:00:00Z` < `T10:00:00Z`）
- 显示层已确认只展示到天，用户不会看到时间部分
