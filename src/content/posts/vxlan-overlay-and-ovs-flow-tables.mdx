...
| VLAN（Virtual Local Area Network） | 在交换机上通过端口划分实现二层隔离的技术，ID 为 12 位，上限 4096 个 |
...
VLAN 可以在同一台交换机上划分不同网段，但其 ID 只有 12 位，上限 4096 个，且受限于二层网络无法跨越路由器。VXLAN（Virtual eXtensible LAN，RFC 7348）用封装技术解决了这两个限制：
