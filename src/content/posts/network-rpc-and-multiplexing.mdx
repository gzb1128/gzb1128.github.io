...
既然 TCP 不提供消息边界（见上文起点的第一个问题），协议必须自己切。解决办法是经典的长度前缀：发送方先写一个长度字段，再写负载；接收方先读长度，再按长度精确读够负载。既不丢字节，也不会读到下一条消息的地盘。

一个帧的线上格式如下，长度字段用大端序（网络协议惯例，收发双方约定同一字节序即可）。
...
Type 字段标明这条消息的用途：REQUEST（方法调用请求）、RESPONSE（unary 调用的最终返回）、DATA（流中的一条数据）、CLOSE（流结束信号，对应读到结尾）。读侧拿到帧后先看 Type 才决定如何处理，这是机制二分发的前提。帧化只保证"一整条消息完整到达"，它不关心负载里装的是什么——这一点会在四层模型里再次出现。
...
```
   ...
   └────────────────────────────────────────
              ...
              [queue]      [queue]      [queue]      (buffered channels)
                  │            │            │
            waiter A     waiter B     waiter C       (each blocks on its own Recv)
```
...
```
multiplexed: N streams on 1 TCP
  stream A ┤ A.req ─wait─► A.resp        A/B/C 互不阻塞 (connection-level parallel)
  stream B ┤ B.req ─wait─► B.resp        but inside A: still one-request-one-response
...
│  wire: [len][id=B][DATA][33%][len][id=B][DATA][66%][len]...
│  framework only asks: is this a complete frame?
└──────────────────────────────────────────────────────────────
                          │ ReadFrame() slices frames
                          ▼
┌─ Layer 2: logical-stream dispatch (framework's job)
│  readLoop delivers each frame by StreamID into per-stream queues
│    stream#A queue: [REQUEST] ... [RESPONSE]   main call
│    stream#B queue: [DATA][DATA][DATA][CLOSE]  progress stream
...
└──────────────────────────────────────────────────────────────
                          │ DATA frames are just carried along
                          ▼
...
把上述三个机制拼成一个可运行的整体，是一个有效的巩固方式。我为此维护了一个教学项目 [minirpc](https://github.com/gzb1128/minirpc)：从零写一个极简 RPC 框架，只用 Go 标准库，零外部依赖，配三个 demo 把三个机制分别跑出来。
...
minirpc 的帧对应 containerd 内部 RPC 协议 ttrpc 的消息帧，Conn 按 StreamID 分发对应 ttrpc 的流多路复用。demo 里学到的机制与生产级 RPC 一致，生产版只是多了 protobuf、TLS、完善的错误处理等工程外壳。如果想在读完本文后把这些机制亲手跑一遍、改一改参数看时序变化，minirpc 是一个直接可用的样本。
