...
```
gRPC on the wire (one HTTP/2 DATA frame):

┌─ HTTP/2 frame header (9 B, by HTTP/2 spec) ────────────────────┐
│ length | type=DATA | flags | stream id                         │
│   ^ stream id = multiplex key  (Layer 1 + 2)                   │
└────────────────────────────────────────────────────────────────┘

┌─ gRPC Length-Prefixed-Message (DATA payload) ──────────────────┐
│ compressed-flag(1B) | message-length(4B BE)                    │
│   ^ gRPC's own length prefix                                   │
│                                                                │
│   +--------------------------------------+                     │
│   | protobuf-serialized message bytes    |                     │
│   +--------------------------------------+                     │
│   ^ Layer 3 + 4 content (the message body)                     │
└────────────────────────────────────────────────────────────────┘
```
