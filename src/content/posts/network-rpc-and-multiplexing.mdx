...
```mermaid
flowchart TB
    subgraph H2["HTTP/2 frame header (9 B, by HTTP/2 spec)"]
        direction TB
        H2F["length | type=DATA | flags | stream id"]
        H2N["^ stream id = multiplex key  (Layer 1 + 2)"]
    end
    subgraph GRPC["gRPC Length-Prefixed-Message (DATA payload)"]
        direction TB
        GF["compressed-flag (1B) | message-length (4B BE)"]
        GN["^ gRPC's own length prefix"]
        PB["protobuf-serialized message bytes"]
        PBN["^ Layer 3 + 4 content (the message body)"]
    end
    H2 --- GRPC
    GN -.- PB
```
