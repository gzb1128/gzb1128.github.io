...
```mermaid
flowchart TB
    subgraph H2["HTTP/2 frame header (9 B, by HTTP/2 spec) — Layer 1 + 2"]
        direction TB
        H2F["length | type=DATA | flags | stream id<br/><i>stream id = multiplex key</i>"]
    end
    subgraph GRPC["gRPC Length-Prefixed-Message (DATA payload)"]
        direction TB
        GF["compressed-flag (1B) | message-length (4B BE)<br/><i>gRPC's own length prefix</i>"]
        PB["protobuf-serialized message bytes<br/><i>Layer 3 + 4 content</i>"]
    end
    H2 --- GRPC
    GF -.- PB
```
