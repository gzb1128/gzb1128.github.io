...
前文已经分别分析了 [Harness 的 Pipeline 并发](/posts/cloud-native/harness-pipeline-concurrency/)和 [Terraform 的 State 与 Dependency Graph](/posts/cloud-native/terraform-state-graph-concurrency/)。三者可以放在同一坐标系中理解：
...
这也补充了 [交付平台参考模型](/posts/cloud-native/delivery-platform-industry-patterns/)中的结论：通用机制可以负责触发、队列和执行，真正影响交付正确性的冲突域、不可变输入与目标归约，最终仍要由最理解资源的一层承担。
